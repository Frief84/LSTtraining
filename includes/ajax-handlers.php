...
add_action("wp_ajax_lsttraining_save_einsatzgebiet", function () {
    ...
    lsttraining_log_activity(['entity_type' => 'leitstelle', 'action' => 'update', 'entity_id' => (int)$leitstelle_id, 'meta' => ['field' => 'geojson', 'page' => 'ajax:save_einsatzgebiet']]);
...
add_action("wp_ajax_lsttraining_save_neben_einsatzgebiet", function () {
    ...
    lsttraining_log_activity(['entity_type' => 'nebenstelle', 'action' => 'update', 'entity_id' => (int)$neben_id, 'meta' => ['field' => 'geojson', 'page' => 'ajax:save_neben_einsatzgebiet']]);
    wp_send_json_success();
});
/**
...
add_action("wp_ajax_lsttraining_save_nebenleitstelle", function () {
    ...
    try {
        if ($id > 0) {
           ...
           // Altstand für Diff laden (nur relevante Spalten)
            $old = [];
            try {
                $sel = $pdo->prepare("
            ...
            $stmt = $pdo->prepare("
                UPDATE nebenleitstellen
                 ...
                 WHERE id = :id
            ");
            $stmt->execute([":name" => $name, ":zust" => $zust, ":einwohner" => $einwohner, ":flaeche_km2" => $flaeche_km2, ":gps" => $gps, ":id" => $id, ]);
            wp_send_json_success(["id" => $id]);

		// NEU: exakte Feld-Diffs protokollieren
            $new = [
                'name'           => $name,
                'zustandigkeit'  => $zust,
                'einwohner'      => $einwohner,
            ...
            // Normalisierung, um "0" vs 0, whitespace etc. sauber zu vergleichen
            $norm = function($v) {
                if ($v === null) return null;
                if (is_bool($v)) return $v ? '1' : '0';
                if (is_float($v) || is_numeric($v)) {
                    // floats auf 6 Nachkommastellen normalisieren
            ...
            if ($desired_id > 0) {
                ...
                ");
                $stmt->execute([":id" => $desired_id, ":name" => $name, ":zust" => $zust, ":einwohner" => $einwohner, ":flaeche_km2" => $flaeche_km2, ":gps" => $gps, ]);
                wp_send_json_success(["id" => $desired_id]);
				
				 // NEU: angelegte Felder protokollieren
                lsttraining_log_activity([
                    'entity_type' => 'nebenstelle',
                    'action'      => 'create',
                ...
                ");
                $stmt->execute([":name" => $name, ":zust" => $zust, ":einwohner" => $einwohner, ":flaeche_km2" => $flaeche_km2, ":gps" => $gps, ]);
 ...
 */
function lst_col_exists(PDO $pdo, string $table, string $col):
    bool {
        try {
...
";
    ...
    add_action("wp_ajax_lsttraining_get_wache", function () {
        ...
        try {
            ...
            $stmt = $pdo->prepare('SELECT id, name, typ, latitude, longitude,
    ...
    add_action("wp_ajax_lsttraining_save_wache", function () {
      ...
      WHERE id = :id',);
            ...
            $pdo->commit();
            // Log: Wache aktualisiert
            lsttraining_log_activity(['entity_type' => 'wache', 'action' => 'update', 'entity_id' => (int)$id, 'meta' => ['page' => 'ajax:save_wache']]);
            wp_send_json_success();
        }
        catch(Exception $e) {
            $pdo->rollBack();
    ...
    add_action("wp_ajax_lsttraining_delete_wache", function () {
        ...
        try {
            ...
            $pdo->commit();
            lsttraining_log_activity(['entity_type' => 'wache', 'action' => 'delete', 'entity_id' => (int)$id, 'meta' => ['page' => 'ajax:delete_wache']]);
            wp_send_json_success();
        }
        catch(Exception $e) {
            $pdo->rollBack();
    ...
    add_action("wp_ajax_lsttraining_create_wache", function () {
        ...
        try {
            ...
            $stmt = $pdo->prepare('INSERT INTO wachen
         ...
         VALUES
            ...
            $pdo->commit();
            lsttraining_log_activity(['entity_type' => 'wache', 'action' => 'create', 'entity_id' => (int)$new_id, 'meta' => ['page' => 'ajax:create_wache']]);
    ...
    add_action("wp_ajax_get_krankenhaeuser", function () {
        ...
        $pdo = lsttraining_get_connection();
        $stmt = $pdo->prepare('SELECT id, name, versorgungsstufe, trauma_level, latitude, longitude
           FROM krankenhaeuser
          ORDER BY name',);
        $stmt->execute();
    ...
    function lsttraining_ajax_get_krankenhaus() {
        ...
        $pdo = lsttraining_get_connection();
        $stmt = $pdo->prepare('SELECT id, name, versorgungsstufe, trauma_level,
    ...
    add_action("wp_ajax_delete_krankenhaus", function () {
        ...
        $ok = $stmt->execute([$id]);
        $ok ? wp_send_json_success() : wp_send_json_error("Löschen fehlgeschlagen", 500);
    });
    /**
    ...
    add_action("wp_ajax_save_krankenhaus", function () {
        ...
        $departments_in = array_key_exists("departments", $_POST) ? stripslashes($_POST["departments"]) : null; // bleibt NULL bei Edit ohne Änderungen
        /* ---------- Metadaten ---------- */
        ...
        last_update      = ?,
        last_editor      = ?';
        $params = [$name, $versorgungsstufe, $trauma_level, $latitude, $longitude, $helipad, $now_mysql, // nur nötig, wenn last_update NICHT per ON UPDATE gesetzt
        $editor_id, ];
    ...
    add_action("wp_ajax_lsttraining_create_krankenhaus", function () {
        ...
        /* 4 | Einfügen  ----------------------------------------------------- */
        $pdo = lsttraining_get_connection();
        $stmt = $pdo->prepare('INSERT INTO krankenhaeuser
        ...
        /* 5 | Antwort  ------------------------------------------------------ */
        if ($ok) {
            wp_send_json_success(["new_id" => (int)$pdo->lastInsertId(), ]);
        }
        wp_send_json_error("Anlegen fehlgeschlagen", 500);
    });
    /* -------------------------------------------------------------------------
    ...
    function lsttraining_save_departments() {
        ...
        foreach ($raw as $key => $val) {
            ...
            if (is_array($val) && isset($val["Lat"], $val["Long"])) {
                ...
                if ($code !== "") {
                    $map[$code] = ["Lat" => floatval($val["Lat"]), "Long" => floatval($val["Long"]), ];
                }
                continue;
            }
            ...
            if (is_array($val) && isset($val["code"])) {
                ...
                if ($code !== "") {
                    $map[$code] = ["Lat" => floatval($val["latitude"]??$defLat), "Long" => floatval($val["longitude"]??$defLon), ];
                }
            }
        }
        if (empty($map)) {
        ...
        /* 6 | DB-Update */
        $pdo = lsttraining_get_connection();
        $stmt = $pdo->prepare('UPDATE krankenhaeuser
            SET departments = ?
          WHERE id = ?',);
    ...
    function lsttraining_get_departments() {
        ...
        require_once plugin_dir_path(__FILE__) . '/db.php';
        $pdo = lsttraining_get_connection();
        $stmt = $pdo->prepare('SELECT departments, latitude, longitude
           FROM krankenhaeuser
          WHERE id = :hid');
        ...
        foreach ($existing_raw as $item) {
            if (is_array($item) && count($item) === 1 && isset($item[array_key_first($item) ])) {
                ...
                $existing[] = ['code' => strtoupper($item['code']), 'latitude' => (float)($item['latitude']??$row['latitude']), 'longitude' => (float)($item['longitude']??$row['longitude']) ];
            } elseif (is_string($item) && $item !== '') {
                $existing[] = ['code' => strtoupper($item), 'latitude' => (float)$row['latitude'], 'longitude' => (float)$row['longitude']];
            }
        }
        ...
        wp_send_json_success(['hospital_id' => $hid, 'existing' => $existing, // [{code, latitude, longitude}, ...]
        ...
        'hospital_lat' => (float)$row['latitude'], 'hospital_lon' => (float)$row['longitude'], ]);
    }
    /* -------------------------------------------------------------------------
    ...
    add_action("wp_ajax_get_leitstelle_hospitals", function () {
        ...
        try {
            // Leitstelle
            $stmt = $pdo->prepare('SELECT available_hospitals,
            ...
            // alle KH für Liste
            $stmt3 = $pdo->query('SELECT id, name, latitude, longitude
               FROM krankenhaeuser
              ORDER BY name',);
            $hospitals = $stmt3->fetchAll(PDO::FETCH_ASSOC);
            wp_send_json_success(["leitstelle_id" => $id, "existing" => $existing, "hospitals" => $hospitals, "leitstelle_lat" => (float)$row["leitstelle_lat"], "leitstelle_lon" => (float)$row["leitstelle_lon"], "geojson" => $row["geojson"], ]);
        }
        catch(PDOException $e) {
    ...
    add_action("wp_ajax_save_leitstelle_hospitals", function () {
        ...
        $pdo = lsttraining_get_connection();
        try {
            $stmt = $pdo->prepare('UPDATE leitstellen
    ...
    add_action("wp_ajax_lsttraining_get_user_permissions", function () {
        ...
        $perms_by_user = [];
        foreach ($rows as $r) {
            $perms_by_user[(int)$r["user_id"]] = ["leitstellen" => (int)$r["can_edit_leitstellen"], "nebenstellen" => (int)$r["can_edit_nebenstellen"], "hospitals" => (int)$r["can_edit_hospitals"], "wachen" => (int)$r["can_edit_wachen"], "fahrzeuge" => (int)$r["can_edit_fahrzeuge"], "leistellen_ids" => $r["leistellen_ids"], ];
        }
        $result = [];
        foreach ($wp_users as $u) {
            $uid = (int)$u->ID;
            $result[] = ["ID" => $uid, "user_login" => $u->user_login, "display_name" => $u->display_name ? : $u->user_login, "permissions" => $perms_by_user[$uid]??["leitstellen" => 0, "nebenstellen" => 0, "hospitals" => 0, "wachen" => 0, "fahrzeuge" => 0, "leistellen_ids" => "", ], ];
        }
    ...
    add_action("wp_ajax_lsttraining_save_user_permissions", function () {
        ...
        $stmtInsert = $pdo->prepare('INSERT INTO user_permissions (
            user_id,
            can_edit_leitstellen,
            can_edit_nebenstellen,
            can_edit_hospitals,
        ...
        $stmtUpdate = $pdo->prepare('UPDATE user_permissions
        ...
        try {
            ...
            foreach ($data as $entry) {
                ...
                $ids_arr = array_filter(array_map("trim", explode(",", $ids_raw)), static fn($v) => $v !== "" && ctype_digit($v),);
                $leistellen_ids = implode(",", $ids_arr);
                $stmtCheck->execute([$user_id]);
                $exists = (bool)$stmtCheck->fetchColumn();
                if ($exists) {
                    $stmtUpdate->execute([$can_leitstellen, $can_nebenstellen, $can_hospitals, $can_wachen, $can_fahrzeuge, $leistellen_ids, $user_id, ]);
                } else {
                    $stmtInsert->execute([$user_id, $can_leitstellen, $can_nebenstellen, $can_hospitals, $can_wachen, $can_fahrzeuge, $leistellen_ids, ]);
                }
            }
            $pdo->commit();
            wp_send_json_success();
    ...
    function lsttraining_ajax_copy_leitstelle() {
        ...
        ");
            $insert->execute([":nid" => $neben_id, ":lid" => $leit_id, ]);
        ...
        ");
            ...
            UPDATE nebenleitstellen
             ...
             WHERE id      = :nid
        ");
            $upd->execute([":gps" => $gps, ":geo" => $row["geojson"], ":nid" => $neben_id, ]);
    ...
    function lsttraining_find_wachen_in_polygon_core_pdo(PDO $pdo, string $type, int $id, ? string $geoOverride = null,) : array {
        ...
        $stmtW = $pdo->prepare('SELECT id, latitude, longitude
           FROM wachen
          WHERE longitude BETWEEN :minLon AND :maxLon
            AND latitude  BETWEEN :minLat AND :maxLat',);
        $stmtW->execute([":minLon" => $minLon, ":maxLon" => $maxLon, ":minLat" => $minLat, ":maxLat" => $maxLat, ]);
    ...
    function lst_find_ids_via_geo_override(string $type, int $id):
        array {
            $geojson = trim(stripslashes($_POST["geojson"]??""));
            $pdo = lsttraining_get_connection();
            if ($geojson === "") {
        ...
        add_action('wp_ajax_lsttraining_update_wache', function () {
            ...
            try {
              ...
              WHERE id = :id');
                ...
                // Log: Relation geändert
                lsttraining_log_activity(['entity_type' => 'wache', 'action' => 'update', 'entity_id' => (int)$wacheId, 'meta' => ['relation' => ($type === 'leitstelle' ? 'wache_leitstelle' : 'wache_nebenleitstelle'), 'owner_id' => (int)$ownerId, 'assigned' => (bool)$assign, 'page' => 'ajax:toggle_wache_assignment']]);
                wp_send_json_success(["wache_id" => $wacheId, "assigned" => (bool)$assign, ]);
            }
            catch(Throwable $e) {
...
add_action('wp_ajax_lsttraining_get_fahrzeug', function () {
    ...
    require_once plugin_dir_path(__FILE__) . 'db.php';
    $pdo = lsttraining_get_connection();
    if (!$pdo) wp_send_json_error(['code'=>'db','msg'=>'DB-Verbindung fehlgeschlagen'], 500);

    $id = (int)($_GET['id'] ?? 0);
    if ($id <= 0) wp_send_json_error(['code'=>'bad_request','msg'=>'ID fehlt'], 400);
    ...
    try {
        ...
        if (!$row) wp_send_json_error(['code'=>'not_found','msg'=>'Nicht gefunden'], 404);
...
add_action('wp_ajax_lsttraining_save_fahrzeug', function () {
    ...
    // Eingaben (Status kommt NICHT vom Client; wird aus FMS abgeleitet)
    $id           = (int)($_POST['id'] ?? 0);
    ...
    $allowed_fms = ['2', '6'];
    if (!in_array($fms_status, $allowed_fms, true)) {
        wp_send_json_error(['code' => 'validation', 'msg' => 'Ungültiger FMS-Status (nur 2 oder 6 erlaubt)'], 400);
    }

    // Status aus FMS ableiten
    // 2 => „frei ab Wache“ → einsatzbereit | 6 => „außerdienst“ → nicht einsatzbereit
    ...
    try {
        if ($id > 0) {
            ...
            // Diff berechnen & Activity loggen (außerhalb der TX)
            $new = [
                'wache_id'           => $wache_id,
                'rufname'            => $rufname,
...
add_action('wp_ajax_lsttraining_upload_fahrzeug_bild', function () {
    ...
    // Akzeptiere sowohl "file" als auch "fz_file" – falls JS mal ändert
    $fileArray = null;
    ...
    if (!$fileArray || empty($fileArray['name']) || empty($fileArray['tmp_name'])) {
        // 200 mit Fehlertext (statt 400), damit das Frontend die Fehlermeldung sauber anzeigen kann
    ...
    // Optional: Limits prüfen – hilfreiche Fehlermeldung
...
add_action('wp_ajax_lsttraining_search_wachen', function () {
    ...
    $sql = "SELECT id, name FROM wachen
            $where_sql
            ORDER BY name ASC
            LIMIT $limit";
