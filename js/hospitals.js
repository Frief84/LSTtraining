...
(function($) {
  ...
  function initDeptTranslateInteraction() {
    ...
    window.deptTranslate = new ol.interaction.Translate({ features: dragCollection });
  ...
  window.initDeptTranslateInteraction = initDeptTranslateInteraction;
    ...
    .done(json => {
	  ...
	  existing:     data.existing,  // bereits vorhandene
	  departments:  data.allowed    // ALLE erlaubten, damit _.each(data.departments) im Template arbeitet
	}));

      // 2) Karte initialisieren (erst jetzt ist #dept-map im DOM)
      ...
      if (!window.deptTranslate && typeof initDeptTranslateInteraction === 'function') {
        initDeptTranslateInteraction();
      }

      // 3) Krankenhaus-Marker
      ...
      // 6) bindDepartmentForm & checkbox-Vorbelegung
      bindDepartmentForm({
        hospital_lat: data.hospital_lat,
        hospital_lon: data.hospital_lon,
      ...
      data.existing.forEach(dep => {
        $container.find(`.dept-toggle[value="${dep.code}"]`)
          .prop('checked', true)
          .trigger('change');
      });
      ...
      // 8) Zusätzliche Department-Marker
      data.existing.forEach(dep => {
        if (!dep.latitude || !dep.longitude) return;
        const feat = new ol.Feature({
          geometry: new ol.geom.Point(
            ol.proj.fromLonLat([+dep.longitude, +dep.latitude])
          ), code: dep.code
        });
